...
class TrainingSpace extends Component {
  ...
  render() {
    const {trainingspace, slug, classes, filters, facets, results} = this.props
    const {hits, total} = results
    return (
      <Paper className={classes.description} elevation={1}>
        <Typography variant="h3" classes={{root: classes.title}}>
          TrainingSpace
          <Link className={classes.seeMoreLink} to={`${slug}/trainingspace`}>
...
const mapStateToProps = ({trainingspace, entity}) => {
  const {slug} = entity
  return {...trainingspace, slug, entity}
}

export default withStyles(styles)(connect(mapStateToProps)(TrainingSpace))
